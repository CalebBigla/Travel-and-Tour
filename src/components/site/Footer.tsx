  ...
  return (
    <footer className="bg-primary-deep text-primary-foreground">
      <div className="container-page grid gap-10 py-14 md:grid-cols-2 lg:grid-cols-4">
        <div>
          <p className="font-display text-xl font-extrabold tracking-[0.18em]">{site.shortName.toUpperCase()}</p>
          <p className="mt-1 text-xs font-semibold uppercase tracking-[0.22em] text-accent">
            Travel &amp; Tours
          </p>
          <p className="mt-4 text-sm leading-relaxed text-primary-foreground/70">
            Visas processed, tours designed, and travel made easy. One trusted agency, two specialized divisions.
          </p>
        ...
        <div>
          <h3 className="text-sm font-bold uppercase tracking-widest text-accent">Tour Services</h3>
          <ul className="mt-4 space-y-2 text-sm text-primary-foreground/75">
            {tourServices.map((s) => (
              <li key={s.slug}>
                <Link to="/tours" hash={s.slug} className="hover:text-accent">
                  {s.title}
                </Link>
              </li>
            ))}
